...
const selectTodo = (url) => {
  ...
  fetch(url)
    // 자바 객체를 JS 객체로 변환하기 위해서 json 사용
    .then( resp => resp.json() )
    .then( todo => {

      // popupLayer 에 조회해온 값을 출력
...
deleteBtn.addEventListener("click", () => {
  ...
  fetch("/ajax/delete?todoNo=", {
    ...
    // -> 원래는 이렇게 명시하는 것이 옳음(엄격한 환경에서는 꼭 명시)   
  })
  .then( resp => resp.text() ) // 1 또는 0 으로 응답 받을 경우 text 사용
  .then( result => {
...
changeComplete.addEventListener("click", () => {
  ...
  // 현재 완료여부를 반대 값으로 변경한 값, 변경할 할 일 번호
  const complete = popupComplete.innerText === 'Y' ? 'N' : 'Y';
  const todoNo = popupTodoNo.innerText;

  // SQL 수행에 필요한 두 값을 JS 객채로 묶음
